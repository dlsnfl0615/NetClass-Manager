...
관리자가 웹 대시보드를 통해 원격으로 시스템을 제어하고, **재부팅 시 PC 상태를 특정 시점(Snapshot)으로 자동 복구(Rollback)**하는 기능을 핵심으로 합니다.
...
* **구역별 현황:** `GROUP BY ... WITH ROLLUP`을 활용하여 층별, 장소별 PC 대수의 소계(Sub Total)와 총계(Total)를 제공합니다.
